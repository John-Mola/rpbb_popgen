...
df_rpbb_sitecenters <- readRDS("./data/data_output/output_01c_df_cluster10_centroids.Rdata") %>% 
  mutate(site_year = paste0(cluster10, "_", year))
...
p_map <- ggplot() +
  ...
  new_scale_fill() +
  geom_point(data = df_rpbb_sitecenters, aes(x = longitude_center, y = latitude_center, fill = as.factor(named_cluster100), size = n), alpha = 0.7, shape = 21) +
...
ggsave(p_map, filename = "./figures/figures_output/04a_supplement_collections_site_map.svg")
